...
const toRaw = ( url ) => {
	...
	if ( isTree( url ) ) {
		url = url.substr( -1 ) === '/' ? `${ url }readme.md` : `${ url }/readme.md`;
		return url.replace( replaceTree, ( _, base, __, path ) => `${ base }raw${ path }` )
	}
	if ( isBlob( url ) ) {
		if ( isFolder( url ) ) {
			url = url.substr( -1 ) === '/' ? `${ url }readme.md` : `${ url }/readme.md`;
		...
		if ( isMarkdown( url ) ) {
			return url.replace( replaceBlob, ( _, base, __, path ) => `${ base }raw${ path }` );
		}
	}
